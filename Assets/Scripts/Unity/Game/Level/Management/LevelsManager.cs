...
namespace Soko.Unity.Game.Level.Management
{
    public class LevelsManager : IInitializable
    {
        ...
        private void FetchLevelPackDatas() => _levelPacks = _levelPacksSo.LevelPacks.Select(so => so.LevelPack).ToList();
        ...
        {
            LevelPackIndex = SaveData.PackSaveDatas.Count - 1;
            LevelIndex = SaveData.PackSaveDatas[LevelPackIndex].Levels.Count - 1;
        }
        ...
        public LevelState CheckLevelState(int packIndex, int levelIndex)
        {
            ...
            if (packIndex == SaveData.PackSaveDatas.Count - 1 &&
                SaveData.PackSaveDatas.Count == _levelPacks.Count &&
                levelIndex == SaveData.PackSaveDatas[packIndex].Levels.Count - 1 &&
        ...
        private void UnlockNextLevel(LevelPackSaveData currentPackSaveData)
        {
            ...
            else if (LevelPackIndex < _levelPacks.Count - 1)
            {
                LevelPackIndex++;
                LevelIndex = 0;
